...
class LiveTrackingController
  ...
  Rx<DriverUserModel> driverUserModel =
      DriverUserModel().obs;
  Rx<OrderModel> orderModel = OrderModel().obs;
  Rx<InterCityOrderModel> intercityOrderModel =
  ...
  void refreshTrackingNow() {
    try {
      _updateTrackingLogic(force: true);
    } catch (e) {
      debugPrint("refreshTrackingNow error: $e");
    }
  }
    ...
    try {
      if (argumentData != null) {
        ...
        if (type.value == "orderModel") {
          OrderModel o =
              argumentData['orderModel'];
          orderId = o.id ?? "";
          initialDriverId = o.driverId;
          orderModel.value = o;
        } else {
          InterCityOrderModel o =
        ...
        orderSubscription = FireStoreUtils
            .fireStore
            .collection(type.value == "orderModel"
  ...
  void _addBaseMarkers() {
    double? lat;
    double? lon;
    String status = type.value == "orderModel"
        ? orderModel.value.status ?? ""
        : intercityOrderModel.value.status ?? "";

    if (type.value == "orderModel") {
      if (status == Constant.rideInProgress) {
        lat = orderModel.value
            .destinationLocationLAtLng?.latitude;
        lon = orderModel.value
      ...
      if (status == Constant.rideInProgress) {
        lat = intercityOrderModel.value
            .destinationLocationLAtLng?.latitude;
        lon = intercityOrderModel.value
            .destinationLocationLAtLng?.longitude;
        if (lat != null && lon != null)
          addMarker(
              latitude: lat,
...
*/
  ...
  void _updateTrackingLogic(
    ...
    if (driverLat == null || driverLon == null) {
      debugPrint("Driver location is null");
      return;
    }
    ...
    if (type.value == "orderModel") {
      if (status == Constant.rideInProgress) {
        targetLat = orderModel.value
            .destinationLocationLAtLng?.latitude;
        targetLon = orderModel.value
            .destinationLocationLAtLng?.longitude;
      } else if (status == Constant.rideActive) {
        targetLat = orderModel
            .value.sourceLocationLAtLng?.latitude;
        targetLon = orderModel.value
            .sourceLocationLAtLng?.longitude;
      }
      ...
      if (status == Constant.rideInProgress) {
        targetLat = intercityOrderModel.value
            .destinationLocationLAtLng?.latitude;
        targetLon = intercityOrderModel.value
            .destinationLocationLAtLng?.longitude;
      } else if (status == Constant.rideActive) {
        targetLat = intercityOrderModel
            .value.sourceLocationLAtLng?.latitude;
        targetLon = intercityOrderModel.value
            .sourceLocationLAtLng?.longitude;
      }
    ...
    if (targetLat == null ||
        ...
        targetLat == 0.0 ||
        targetLon == 0.0) {
      debugPrint("Target location invalid");
      return;
    }

    // Always update direct distance first so UI card never stays empty
    ...
    distance.value = (directDistanceMeters / 1000)
        .toStringAsFixed(2);

    // Throttle only API-heavy updates, not initial forced refresh
    if (!force && lastDriverLocation != null) {
      final double moved =
          geo.Geolocator.distanceBetween(
        lastDriverLocation!.latitude,
        lastDriverLocation!.longitude,
      ...
      if (moved < 10) {
        // Even if driver hasn't moved much, keep markers visible
        _drawFallbackState(
          driverLat: driverLat,
          driverLon: driverLon,
          targetLat: targetLat,
    ...
    getPolyline(
      ...
      sourceLongitude: driverLon,
      destinationLatitude: targetLat,
      destinationLongitude: targetLon,
    );
  }

  void _drawFallbackState({
    ...
    // Fallback straight polyline so route is still visible
  ...
  Future<void> addMarkerSetup() async {
    if (argumentData != null) {
      type.value = argumentData['type'];
      if (type.value == "orderModel") {
        OrderModel argumentOrderModel =
            argumentData['orderModel'];
        serviceMarkerIcon.value =
            argumentOrderModel
                    .service?.markerIcon ??
                '';
      } else {
        InterCityOrderModel
            argumentOrderCityModel =
            argumentData['interCityOrderModel'];
        final drivermodel =
            await FireStoreUtils.getDriver(
                argumentOrderCityModel.driverId!);
        final service =
            await FireStoreUtils.getServiceById(
      ...
      final Uint8List departure = await Constant()
          .getBytesFromAsset(
              'assets/images/pickup.png', 120);
      final Uint8List destination =
          await Constant().getBytesFromAsset(
              'assets/images/dropoff.png', 120);
      final Uint8List driver = serviceMarkerIcon
                  .value ==
              ''
          ? await Constant().getBytesFromAsset(
              'assets/images/ic_cab.png', 50)
          : await Constant().getBytesFromUrl(
              serviceMarkerIcon.value,
              width: 120);
      departureIcon =
          BitmapDescriptor.fromBytes(departure);
      destinationIcon =
          BitmapDescriptor.fromBytes(destination);
      driverIcon =
  ...
  void getPolyline({
    ...
    // Always show direct distance immediately
    final double directDistance = geo.Geolocator.distanceBetween(
      sourceLatitude,
      sourceLongitude,
      destinationLatitude,
    ...
    try {
      final url = Uri.parse(
        'https://maps.googleapis.com/maps/api/directions/json?origin=$sourceLatitude,$sourceLongitude&destination=$destinationLatitude,$destinationLongitude&key=${Constant.mapAPIKey}',
      );
      ...
      if (route['overview_polyline'] == null ||
        ...
        debugPrint("No overview polyline found");
        return;
      }

      final String points = route['overview_polyline']['points'];
      final List<LatLng> polylineCoordinates = _decodeEncodedPolyline(points);

      if (legs != null && legs.isNotEmpty && legs[0]['distance'] != null) {
        final String distText = legs[0]['distance']['text'].toString();
    ...
    if (sourceLatitude != null &&
      ...
      double directDistance =
          geo.Geolocator.distanceBetween(
              ...
              destinationLatitude,
              destinationLongitude);
      distance.value = (directDistance / 1000)
          .toStringAsFixed(2);
        ...
        if (response.statusCode == 200) {
          ...
          if (decoded['status'] == 'OK') {
            ...
            List<LatLng> polylineCoordinates =
                _decodeEncodedPolyline(points);

            // Get distance from legs
            String distText = route['legs'][0]
                ['distance']['text'];
            distance.value = distText.replaceAll(
                RegExp(r'[^0-9.]'), '');

            markers.clear();
            polyLines.clear();
            String status = type.value ==
                    "orderModel"
                ? orderModel.value.status ?? ""
                : intercityOrderModel
                        .value.status ??
            ...
            addMarker(
                ...
                id: "Driver",
                descriptor: driverIcon!,
                rotation: driverUserModel
                    .value.rotation);
            _addPolyLine(polylineCoordinates);
          } else {
            debugPrint(
                "Google Directions API error: ${decoded['status']}");
          }
        }
      } catch (e) {
        debugPrint(
            "Error fetching polyline using Google HTTP: $e");
      }
    }
  }*/
  ...
  Future<void> updateCameraLocation(
    ...
    if (source.latitude > destination.latitude &&
      ...
      bounds = LatLngBounds(
          ...
          northeast: source);
    } else if (source.longitude >
        destination.longitude) {
      bounds = LatLngBounds(
          southwest: LatLng(source.latitude,
              destination.longitude),
          northeast: LatLng(destination.latitude,
              source.longitude));
    } else if (source.latitude >
        destination.latitude) {
      bounds = LatLngBounds(
          southwest: LatLng(destination.latitude,
              source.longitude),
          northeast: LatLng(source.latitude,
              destination.longitude));
    } else {
      bounds = LatLngBounds(
          southwest: source,
  ...
  List<LatLng> _decodeEncodedPolyline(
      String encoded) {
    List<LatLng> poly = [];
